...
if (argv.help) {
	...
	Arguments:
		...
		--help - show help
		--custom - use custom build config from CUSTOM_BUILD
...
await spinner('[3/5] Getting and processing styles...', async () => {
	if (argv.custom) {
		// Set custom theming values
		echo`Applying custom theming: primary_color=${BUILD_CONFIG.primaryColor}, background_color=${BUILD_CONFIG.backgroundColor}, no background`
		await $`docker exec -u www-data ${CONTAINER_NAME} php /var/www/nextcloud/occ config:app:set theming primary_color --value=${BUILD_CONFIG.primaryColor}`
		await $`docker exec -u www-data ${CONTAINER_NAME} php /var/www/nextcloud/occ config:app:set theming background_color --value=${BUILD_CONFIG.backgroundColor}`
		...
		await $`docker exec -u www-data ${CONTAINER_NAME} php /var/www/nextcloud/occ config:app:delete theming primary_color`
		await $`docker exec -u www-data ${CONTAINER_NAME} php /var/www/nextcloud/occ config:app:delete theming background_color`
		await $`docker exec -u www-data ${CONTAINER_NAME} php /var/www/nextcloud/occ config:app:delete theming backgroundMime`
	}
